// Content script for ChatGPT chat extraction
class ChatGPTExtractor {
  constructor() {
    ...
    this.log('ChatGPT content script loaded');
    this.init();
  }
  ...
  init() {
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('DOMContentLoaded', () => {
        setTimeout(() => this.startExtraction(), 3000); // Wait 3 seconds after DOM ready
      });
    } else {
      setTimeout(() => this.startExtraction(), 3000); // Wait 3 seconds
    }
  }
  ...
  extractExistingMessages() {
    ...
    console.log('🌀 Twirl: Looking for messages...');
    ...
    for (const selector of messageSelectors) {
      ...
      console.log(`🌀 Twirl: Selector "${selector}" found ${messages.length} elements`);
      if (messages.length > 0) break;
    }

    if (messages.length === 0) {
      // More aggressive fallback search
      console.log('🌀 Twirl: Trying fallback selectors...');
      ...
      for (const selector of fallbackSelectors) {
        messages = document.querySelectorAll(selector);
        console.log(`🌀 Twirl: Fallback selector "${selector}" found ${messages.length} elements`);
        if (messages.length > 2) break; // Need at least a few messages
      }
    }

    this.log(`Processing ${messages.length} message elements`);
  ...
  generateChatId() {
    ...
    return `${urlPath}_${timestamp}`.replace(/[^a-zA-Z0-9_]/g, '_');
  }
